<script type="text/javascript">	
	...
	$(function(){
		$('input#btn_save').click(function(){
			process_form('input#btn_save','SAVE','div#payment_form_messages',true,'form#payment_form','_admin_console/payment/save_payment_plan/<?=$plan['id']?>',false,'save_success');
		});
	...
	function show_payments(el){
		var val = $(el).val();
		if(val == 2){
			$('#initial_amount').val('<?=$plan['initial_payment']?>');
			$('#initial_amount').attr('disabled','');
			$('#recurring_amount').val('<?=$plan['recurring_payment']?>');
	...
	<form id="payment_form">
		...
		<select id="payment_type" onchange="show_payments(this)" class="medium_input_text" name="type" >
			...
			<?php foreach($plan_type AS $type): ?>
				<option <?php if($plan['type'] == $type['id']){ echo 'selected="selected"';} ?> value="<?=$type['id']?>"><?=$type['name']?></option>
			<?php endforeach; ?>
		</select><br/><br/>
		<div id="initial_container" style="display:none;">
			Initial Payment<br/>
			<input id="initial_amount" disabled="disabled" type="text" class="medium_input_text" name="initial_amount" value="<?=$plan['initial_payment']?>" /><br/><br/>
		</div>
		<div id="recurring_container" style="display:none;">
			Recurring Payment<br/>
			<input id="recurring_amount" disabled="disabled" type="text" class="medium_input_text" name="recurring_amount" value="<?=$plan['recurring_payment']?>"/><br/><br/>
		</div>
		Payment Plan Name<br/>	
		<input type="text" class="wide_input_text" name="name" value="<?=$plan['name']?>" /><br/><br/>
		Description/Details<br/>
		<textarea name="description" cols="57" rows="10"><?=$plan['description']?></textarea>
		<br/><br><br>
