...
<script type="text/javascript">
	...
	function confirmCancel(link){
		Sexy.confirm('<h1>Cancel</h1><p>Are you sure you want to cancel and go to Article management?</p>',{onComplete: 
	...
	*/
	...
	$(function(){
		$('input#btn_save').click(function(){
			...
			process_form('input#btn_save','SAVE','div#article_form_messages',true,'form#article_form','_admin_console/articles/save_article/<?=$id?>',false,'save_success');
		});
...
<div>
	...
	<form id="article_form">
		Main Image<br>
		<input type="hidden" value="<?=$image?>" id="main_image" name="image"/>
		<input type="file" id="image_uploadify" name="Filedata" /><br/>
		<?php if($image){ ?>
			<img id="article_main_image" class="main_image" src="<?=base_url();?>content_assets/images/<?=$image?>" alt="image" width="320" height="167" />
		<?php }else{ ?>
		...
		<br/><br/>
		Author<br/>	
		<input type="text" value="<?=$author?>" style="width: 330px;" class="medium_input_text" name="author" /><br/><br/>
		Category<br/>
		...
		Summary<br/>
		<textarea class="wide_input_text"  id="limit_summary" name="summary" rows="5"><?=$summary?></textarea><br/><br/>
		Content<br/>
