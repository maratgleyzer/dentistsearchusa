...
class Dentist extends Controller{
	...
	function ajax_search($search=null,$distance=5){
		...
		$search_result_text = str_replace('%searchname%',ucwords(strtolower($h3label)),$search_result_text);
		...
		if($featured){
			foreach($featured AS $dentist){
				$markers[] = array(
					'icon' => '/dentist/featured_marker_label/'.$i,
				...
				$this->googlemaps->get_lat_long_from_address("{$dentist['address']}, {$dentist['city']}, {$dentist['state']} {$dentist['zip']}, USA"),
					'details' => "
								...
								<a href='".base_url().prep_seo_url($dentist['city']).'-'.prep_seo_url($dentist['state'])."-dentists/".prep_seo_url($dentist['first_name'])."-".prep_seo_url($dentist['last_name'])."-".$dentist['id']."' class='marker_details' ><img style='display:inline;margin-bottom:-4px;' src='".base_url()."assets/images/admin/vcard.png'>&nbsp; View Profile</a>"
				);
				$prof_pic = $dentist['prof_pic'];
				if($prof_pic){
					$picvid = explode(".",$prof_pic);
					if($picvid[1]=="flv"){
						$photo = '<a class="flv_player" href="/user_assets/prof_images/'.$prof_pic.'" rel="shadowbox;width=600;height=450"><img src="/user_assets/prof_images/playvid_small.gif" alt="image" width="85" height="85" /></a>';
					}else{
						$photo = '<img src="/assets/phpthumb/resize.php?src=/user_assets/prof_images/'.$prof_pic.'&width=85&height=85" alt="image" />';
					}
				}else{
					$photo = '<img src="/assets/themes/default/images/no_photo_small.png" alt="image" width="85" height="85" />';
				}
				$content .= '<li>
								<div class="photo">
								...
								<div class="info-holder">
									<div class="text-hold">
										<span class="number"><img alt="'.$i.'" src="/dentist/featured_marker_label/'.$i.'" /></span>
										<div class="text">
		...
		if($dentists){
			foreach($dentists AS $dentist){
				...
				$content .= '<li>
								...
								<div class="info-holder">
									<div class="text-hold">
										<span class="number"><img alt="'.$i.'" src="/dentist/marker_label/'.$i.'" /></span>
										<div class="text">
											<h3 class="ttl"><a href="/'.prep_seo_url($dentist['city']).'-'.prep_seo_url($dentist['state']).'-dentists/'.prep_seo_url($dentist['first_name']).'-'.prep_seo_url($dentist['last_name']).'-'.$dentist['id'].'">'.ret_alt_echo(company_output($dentist['first_name'],$dentist['last_name'],$dentist['company_name']),'','',' :').' '.$dentist['first_name'].' '.$dentist['last_name'].' '.$dentist['post_nominal'].'</a></h3>
											<address>
												<span>'.$dentist['address'].'</span>
	...
	function search($search=null,$distance=5,$search_start=0,$limit_per_page =10){
		...
		foreach($dentists AS $dentist){
			$marker = array(); 
			$marker['icon'] = '/dentist/marker_label/'.$i;
		...
		foreach($featured AS $dentist){
			$marker = array(); 
			$marker['icon'] = '/dentist/featured_marker_label/'.$i;
			if (is_numeric($dentist['latitude']) && is_numeric($dentist['longitude']))
				$marker['position'] = "{$dentist['latitude']}, {$dentist['longitude']}";
			else $marker['position'] = "{$dentist['address']}, {$dentist['city']}, {$dentist['state']} {$dentist['zip']}, USA";  
			$marker['infowindow_content'] = ret_alt_echo(company_output($dentist['first_name'],$dentist['last_name'],$dentist['company_name']),'','',' :')."<br/>{$dentist['first_name']} {$dentist['last_name']} {$dentist['post_nominal']} <br/>{$dentist['address']},<br/>{$dentist['city']}, {$dentist['state']} {$dentist['zip']}, USA<br/><a href='".base_url().prep_seo_url($dentist['city'])."-".prep_seo_url($dentist['state'])."-dentists/".prep_seo_url($dentist['first_name'])."-".prep_seo_url($dentist['last_name'])."-".$dentist['id']."' class='marker_details' ><img style='display:inline;margin-bottom:-4px;' src='".base_url()."assets/images/admin/vcard.png'>&nbsp; View Profile</a>";  
		...
		$search_result_text = $this->Seo_Model->get_search_result_text();
		$data['search_result_text'] = str_replace('%searchname%',ucwords(strtolower($h2label)),$search_result_text);
	...
	function marker_label($text){
		...
		$img = 'googlemap_marker_'.$text.'.png';
		...
		image_add_text('/assets/themes/default/images/googlemap_marker.png',$text);
	}
	function featured_marker_label($text){
		header("Cache-Control: private, max-age=15000, pre-check=15000");
		header("Pragma: private");
		header("Expires: " . date(DATE_RFC822,strtotime(" 1 day")));
		$img = 'googlemap_marker_featured_'.$text.'.png'; 
		if (isset($_SERVER['HTTP_IF_MODIFIED_SINCE']) && 
			(strtotime($_SERVER['HTTP_IF_MODIFIED_SINCE']) == filemtime($img))) {
		  // send the last mod time of the file back
		  header('Last-Modified: '.gmdate('D, d M Y H:i:s', filemtime($img)).' GMT', true, 304);
		...
		image_add_text('/assets/themes/default/images/googlemap_marker_featured.png',$text);
	}
	function profile($id){
