<?php
class Registration_Details extends Controller{
	...
	function index(){				
		...
		$header['seo'] = $this->Seo_Model->get(8);
		...
		$data['payment_plans'] = $this->Payments_Model->get_plans(); 
		
		$this->load->view('registration_details_view', $data);
		$this->load->view('popup/login_popup_view');
		$this->load->view('popup/complete_registration_popup_view');  
		$data['signup_text'] =  $this->Seo_Model->get(10);
		$data['dropdown_choices'] = $this->Choices_Model->get_all(); $this->load->view('popup/signup_popup_view',$data);  
