...
class Googlemaps {
	...
	function create_map($mile_radius = NULL)
	{
		...
		if ($this->is_lat_long($this->center)) { // if centering the map on a lat/long
			$this->output_js_contents .= 'myMapLatlng = new google.maps.LatLng('.$this->center.');';
		}else{  // if centering the map on an address
			$lat_long = $this->get_lat_long_from_address($this->center);
			$this->output_js_contents .= 'myMapLatlng = new google.maps.LatLng('.$lat_long[0].', '.$lat_long[1].');';
		}
		...
		if($this->initializeFunction){
			$this->output_js_contents .= $this->initializeFunction;
		}
		
		$circle_overlay = NULL;
		if($mile_radius){
			$circle_overlay = '
				googlemap_circle = new google.maps.Circle({
					map: map,
					radius: (('.$mile_radius.' *1.609344) * 1000), //1000 miles
					center: myMapLatlng,
					fillColor: "#058478",
					fillOpacity: 0.1,
					strokeColor: "#bc0303",
